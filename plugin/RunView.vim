...
"  call Decho("winout<".winout.">")
  ...
  if bufexists(g:runview_filtcmd)
...
"   call Decho("output window<".g:runview_filtcmd."> already exists, appending to it")
   let curwin  = winnr()
   let bufout  = bufwinnr(g:runview_filtcmd)
   exe bufout."wincmd w"
   set ma
   let lastline= line("$")
...
"   call Decho("exe silent %!".filtcmd)
   exe "silent %!".filtcmd
   exe "file ".fnameescape(g:runview_filtcmd)
   let title       = 'RunView '.g:runview_filtcmd.' Output Window'
   let delimstring = "===".strftime("%m/%d/%y %H:%M:%S")."==="
   1
   silent put!=''
   put =''
